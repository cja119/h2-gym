...
def hydrogen_storage(vars):
    return (
        vars["hydrogen_energy"] / vars["eta_H2"]
        + vars["old_hydrogen_storage"]
        - vars["eta_P_C"]
        * (
            (1 - vars["eta_F_C"]) * vars["conversion_energy"]
            + vars["eta_F_C"] * (vars["conversion_energy"] // vars["P_C_U"])
        )
    )  # P_H2/eta_H2 + E'_H2 - P_H2_out
...
def vector_storage(vars):
    return (
        vars["old_vector_storage"] + vars["vector_production"] - vars["vector_power"]
    )  #
...
def shipping_energy(vars):
    return (
        vars["old_shipping_energy"]
        + vars["vector_production"]
        - vars["E_M_U"]
        * ((vars["old_shipping_energy"] + vars["vector_production"]) // (vars["E_M_U"]))
    )
...
def power_lim(vars):
    return (
        vars["conversion_power"]
        + vars["hydrogen_power"]
        - vars["supplemental_power"]
...
def total_disc_lim(vars):
    return (
        vars["old_shipping_energy"]
        + vars["vector_production"]
        - vars["E_M_U"] * (vars["old_ships"] + vars["ship_order"])
    )
