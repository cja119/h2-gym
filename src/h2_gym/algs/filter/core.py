...
class KalmanFilter:
    ...
    def __init__(self, country: str, period: Optional[str], demand_type: Optional[str], path: Optional[str] = None) -> None:
        """
    ...
    def get_data(self, path: Optional[str], country: Optional[str], period: Optional[str] = None,
                 demand_type: Optional[str] = None) -> None:
        ...
        if mask.sum() == 0:
            ...
            print(f"""[NOTE] Demand type {demand_type} not found in the data, defaulting to industry for demand shape.
    ...
    def gen_multi_synth(self,n_sim: int, dur: int) -> ndarray:
        ...
        with warnings.catch_warnings():
            ...
            for i in range(n_sim):
                sampled_state = random.multivariate_normal(results.filtered_state[:, -1], results.filtered_state_cov[:, :, -1])
    ...
    def plot_synth(self, sims):
        ...
        plt.plot(self._data.index, self._data, label='Train Data', color=muted_color('blue'))
        
        indx = [self._data.index[-1] + DateOffset(months=i) for i in range(len(sims[0])+1)]
    ...
    def predict(self, n_pred: int) -> DataFrame:
        ...
        pred_df = DataFrame({'predicted_mean': pred_values, 'lower_ci': conf_int.iloc[:, 0], 'upper_ci': conf_int.iloc[:, 1]}, index=pred_values.index)
    ...
    def check_test(self) -> None:
        ...
        if len(self._test_data) < 12:
            ...
            for i in range(n_sim):
                sampled_state = random.multivariate_normal(self.last_state_mean, 0.5*self.last_state_cov)
                sim = self._init_results.simulate(nsimulations=36, initial_state=sampled_state)
                sims[i] = sim
            
            # Take the mean of the simulations
            sim = sims.mean(axis=0)
            
            indx = [self._train_data.index[-1] + DateOffset(months=1) + DateOffset(months=i) for i in range(36)]
    ...
    def plot(self):
        ...
        forecast_ci = DataFrame({'lower_ci': forecast['lower_ci'], 'upper_ci': forecast['upper_ci']})
        ...
        plt.rcParams['mathtext.fontset'] = 'cm' 
        plt.figure(figsize=(15, 4.5))
        plt.plot(self._train_data.index, self._train_data, label='Train Data', color=muted_color('blue'))
        plt.plot(self._test_data.index[:12], self._test_data[:12], label='Test Data', color=muted_color('green'))
        
        if not self._train_data.empty:
            plt.plot([self._train_data.index[-1], self._test_data.index[0]],
                 [self._train_data.iloc[-1], self._test_data.iloc[0]],
                 color=muted_color('green'), linestyle='--')
        
        plt.plot(forecast_mean.index, forecast_mean, label='Forecast', color=muted_color('red'))
        if not self._train_data.empty:
            plt.plot([self._train_data.index[-1], forecast_mean.index[0]],
                 [self._train_data.iloc[-1], forecast_mean.iloc[0]],
                 color=muted_color('red'), linestyle='--')

        plt.fill_between([self._train_data.index[-1],*forecast_mean.index],
                [self._train_data.iloc[-1],*forecast_ci['lower_ci']],
                [self._train_data.iloc[-1],*forecast_ci['upper_ci']],
                color=muted_color('orange'), alpha=0.3,
                label='95% CI')
