...
def add_constants(node, data):
    if 'constants' in data[node.get_id()]:
        ...
        node['variables'].collection(
            'constants',
            list(data[node.get_id()]['constants'].keys())
        )
...
def add_inlets(node, data):
    if 'inlets' in data[node.get_id()]:
        ...
        node['variables'].collection(
            'inlets',
            [item['name'] for item in data[node.get_id()]['inlets'].values()]
        )
...
def module_loader(current_path, folder_name) -> None:
    ...
    spec = util.spec_from_file_location('node_functions', current_path/folder_name/functions)
