...
class ShippingEnvV1:
    ...
    def __exit__(self, exc_type, exc_value, traceback) -> None:
        ...
        self._filter, self._weather_data = temporal_align(
            self._filter, self._weather_data, randomise=True
        )
        ...
        latent_states = {
            "current_ships": 0,
            "hydrogen_storage": 0.5 * self._fast_data["params"]["hydrogen_storage_capacity"],
            "vector_storage": 0.5 * self._fast_data["params"]["vector_storage_capacity"],
            "conversion_process_state": (
                0.5 * self._fast_data["params"]["conversion_trains_number"] 
    ...
    def step(self, action):
        ...
        for i in range(n_steps):
            ...
            if latent_states["ordered_ship"] >= 0:
                ship_origin.extent([int(normal(
                    value(self._fast_data['params']['mean_ship_arrival_time']),
                    value(self._fast_data['params']['srd_ship_arrival_time'])
                )) for _ in range(latent_states["ordered_ship"])])

            if latent_states["sent_ship"] >= 0:
                ship_destination.extend([int(normal(
                    value(self._slow_data['params']['mean_ship_transit_time']),
                    value(self._slow_data['params']['srd_ship_transit_time'])
            ...
            if 0 in ship_destination:
                ...
                destination_storage += n_arrived_ships * value(self._fast_data['params']['ship_capacity'])
