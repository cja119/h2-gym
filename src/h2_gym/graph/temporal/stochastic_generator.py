...
class StochasticGenerator:
    ...
    def bind_variable(self, node: Node, varname: str, time_duration: Optional[int] = None):
        """
        Binds a variable to the stochastic generator
        """
        if varname in self._varnames:
            raise ValueError(f"Variable {varname} already exists in the stochastic generator")
    ...
    def bind_dataset(self, varname,path: Optional[str] = None, filenames: Optional[str] = None, 
                     dataset: Optional[str] = None, time_duration: Optional[int] = None):
        """
            ...
            for filename in csvs_files:
                ...
                with open(filename) as f:
                    ...
                    for line in f:
                        ...
                        if len(parts) == 2:
                            _, value = parts
                            self._datasets[varname].extend([float(value)]*time_duration)
                        elif len(parts) == 1:
                            value = float(parts[0])
                            self._datasets[varname].extend([float(value)]*time_duration)
    ...
    def update(self):
        ...
        for varname in self._varnames:
            self._target[varname] = self._datasets[varname][self._time % self._maxtimes[varname]]
        
        result = {
            self._node_id[varname]: {
            varname: self._target[varname]
            }
            for varname in self._varnames
        }
